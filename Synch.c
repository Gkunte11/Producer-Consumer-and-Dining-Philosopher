  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Unlock ()
          ...
          interrupt = SetInterruptsTo(DISABLED)     --avoids the race condition 
          if hold != currentThread
            ...
            if threadptr
              ...
              readyList.AddToEnd(threadptr)         --removing the threads from the waiting list and adding them into the ready list
              hold = threadptr                      --attaching the thread to the critical section
            else
              hold = null
            endIf
          endIf
      ...
      method IsHeldByCurrentThread () returns bool      --Will check which thread is holding the critical section     
          if hold == currentThread                
            return true
          endIf
          return false
