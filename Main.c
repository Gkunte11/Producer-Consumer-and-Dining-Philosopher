...
*/
  ...
  function foo (i: int)
      ...
      for j = 1 to 30
        ...
        -- making this programs output more interesting.
        currentThread.Yield ()
      endFor
    endFunction
...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    bempty: Semaphore = new Semaphore   
    bfull: Semaphore = new Semaphore 
    MutexLock: Mutex = new Mutex
    onemutex: Mutex = new Mutex
    MutexLock1: Mutex = new Mutex
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 5
        ...
        bempty.Down()
        MutexLock.Lock()      -- Lock to synchronise access from multiple producers 

        
        -- Add c to the buffer
        buffer [bufferNextIn] = c
        bufferNextIn = (bufferNextIn + 1) % BUFFER_SIZE
        onemutex.Lock()       -- this lock ensures that a 'if' a context 'switch' occurs in the critical section it wont change the already existing values 
        bufferSize = bufferSize + 1

        -- Print a line showing the state
               
        PrintBuffer (c)
        onemutex.Unlock()       --removing the lock on print
  ...
  function Consumer (myId: int)
      ...
      while true
        ...
        bfull.Down()
        MutexLock1.Lock()     -- Lock to synchronize access from multiple consumers
        -- Remove next character from the buffer
        
        c = buffer [bufferNextOut]
        onemutex.Lock()       --Acquiring the lock 'for' print as bufferNextOut is used in the PrintBuffer function
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  -- look at the statuss of the two adjacent philosophers.  (Another way to state
  ...
  class ForkMonitor
    ...
    fields
      ...
      condition: array [5] of Condition    -- initializing the condition variables
    methods
      Init ()
      PickupForks (p: int)
      PutDownForks (p: int)
  ...
  behavior ForkMonitor
    ...
    method PickupForks (p: int)
      ...
      status[p] = HUNGRY
      mon.PrintAllStatus()
      mon.Checking(p)   -- checking 'if' the philosophers neighbours are eating or not  
      if status[p] != EATING
        condition[p].Wait( & mlock)   -- wait function makes the current thread to sleep and implicitly releases the lock
      endIf 
      mlock.Unlock()
    ...
    method Checking(p: int)

      if status[p] == HUNGRY && status[ (p+1) % 5 ] != EATING && status[ (p-1) % 5] != EATING
        status[p] = EATING
        mon.PrintAllStatus()
        condition[p].Signal( & mlock )  -- signal function wakes up the next thread and this thread tries to acquire the lock
